...
public class UsersController(IUnitOfWork work, IMapper mapper, IPhotoService photoService) : ApiControllerBase {
  ...
  public async Task<ActionResult<PaginatedResponse<SimpleUser>>> GetUsers([FromQuery] GetUsersRequest request) =>
    PaginationInfo.TryCreate(request.ToPage(), await work.Users.CountAsync(request.ToFilter(User.GetUsername())), out var paginationInfo)
      ? Ok(PaginatedResponse<SimpleUser>.FromPaginationInfo(
          paginationInfo,
          await work.Users.GetSimpleUsersAsync(paginationInfo.Page, request.ToFilter(User.GetUsername()), request.OrderBy)))
      // don't shout
  ...
  public async Task<ActionResult<SimpleUser>> GetUser(uint id) 
    => await work.Users.GetDbUserAsync(id) is null ? NotFound() : Ok(await work.Users.GetSimpleUserAsync(id));
  
  [HttpGet("{username}")] 
  public async Task<ActionResult<SimpleUser>> GetUser(string username) 
    => await work.Users.GetSimpleUserAsync(username) is { } simpleUser ? Ok(simpleUser) : NotFound();

  [HttpPut]
  public async Task<ActionResult> UpdateUser(UpdateUserRequest request) {
    if (await work.Users.GetDbUserAsync(User.GetUsername()) is not { } user) 
  ...
  public async Task<ActionResult> DeletePhoto(uint photoId) {
    ...
    return await work.TrySaveAllAsync() ? Ok() : BadRequest("Failed to delete the photo.");
  }
}
